...
2. 抓住关键：1). 微任务队列清空后，才执行宏任务队列; 2).所有可执行同步代码执行完毕后，才执行微任务
3. await xx 是同步的，且会阻塞后面的代码，直到拿到 xx resolve 的结果
```
async function fn(){
    console.log(1)
}
...
* Array.from({length:n},()=>Array.from({length:n},()=>Array.from({length:n},()=>0))) 三维数组初始化
